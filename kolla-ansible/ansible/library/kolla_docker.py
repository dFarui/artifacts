...
class DockerWorker(object):
    ...
    def compare_ipc_mode(self, container_info):
        ...
        if new_ipc_mode is not None and new_ipc_mode != current_ipc_mode:
            # If the actual container is instructed to join to another
            # container's IPC namespace, then the possible member container's
            # name needs to be resolved in order to perform the comparison, as
            # inspect will always return the member container's id.
            if ':' in current_ipc_mode and ':' in new_ipc_mode:
                container_ref = new_ipc_mode.split(':')[1]
                container_id = self.dc.inspect_container(container_ref)['Id']
                new_ipc_mode = "{}:{}".format(new_ipc_mode.split(':')[0],
                                              container_id)
                if new_ipc_mode == current_ipc_mode: return False
            return True
        return False
    ...
    def compare_volumes(self, container_info):
        ...
        # NOTE(GOro): When a container is started with docker CLI, the
        # `Volumes` container configuration option is going to be empty
        # as long as the bind source is given in the arguments for every
        # single volume [1]. Meanwhile, this library compiles an explicit
        # volumes and binds list for the API [2] call, when a container is
        # created by. For this reason, we ease this check.
        # [1] https://github.com/docker/cli/blob/v19.03.1/cli/command/container/opts.go#L340-L353
        # [2] https://docs.docker.com/engine/api/v1.40/#operation/ContainerCreate
        if current_vols and set(volumes).symmetric_difference(set(current_vols)):
            return True
    ...
    def compare_dimensions(self, container_info):
        ...
        dimension_map = {
            ...
            'ulimits': 'Ulimits', 'cgroup_parent': 'CgroupParent'}
    ...
    def parse_dimensions(self, dimensions):
        ...
        supported = {'cpu_period', 'cpu_quota', 'cpu_shares',
                     ...
                     'mem_reservation', 'memswap_limit', 'kernel_memory',
                     'blkio_weight', 'ulimits', 'cgroup_parent'}
...
def generate_module():
    ...
    argument_spec = dict(
        ...
        ipc_mode=dict(required=False, type='str'),
