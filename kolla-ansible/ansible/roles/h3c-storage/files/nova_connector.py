...
class ISCSIConnector(InitiatorConnector):
    ...
    @staticmethod
    def replace_connection_properties_for_iscsi(connection_properties, target_portals):
        """
        strip [ in onestor target portals.
        """
        conn_str = str(connection_properties)
        if -1 == conn_str.find("onestor"):
            return target_portals

        LOG.info("will replace [ ] in iscsi target portal {0}.".format(target_portals))
        src_target_port = str(target_portals).strip('[')
        new_target_port = src_target_port.replace(']', '')
        LOG.info("replaced iscsi target portal is {0}.".format(new_target_port))
        return new_target_port
    ...
    def _disconnect_volume_iscsi(self, connection_properties):
        ...
        #             target are in use.
        target_portals = self.replace_connection_properties_for_iscsi(connection_properties,
                                                                      connection_properties['target_portal'])
        device_byname = ("ip-%(portal)s-iscsi-%(iqn)s-lun-" %
                         {'portal': target_portals,
    ...
    def _disconnect_volume_multipath_iscsi(self, connection_properties,
        ...
        if -1 == conn_str.find("onestor"):
            ...
            for dev in block_devices:
                dev_realpath = os.path.realpath(dev)
                LOG.debug("get real path for {0} is {1}".format(dev, dev_realpath))
                if dev_realpath in mpath_map.keys():
                    devices.append(mpath_map['{0}'.format(dev_realpath)])

        LOG.info("block devices is {0} mpath dev is {1} devices {2}"
                 .format(block_devices, mpath_map, devices))
        ...
        LOG.info("others iqns is {0} current iqns is {1}".format(other_iqns, current_iqns))
        in_use = False
